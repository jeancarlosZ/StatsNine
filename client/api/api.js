...
export async function fetchFullStatement(
  ticker,
  growth = false,
  period = 'annual'
) {
...
//* By Default it returns an annual statement, optionally 'quarter'
//* You can also optionally set growth to true, to only return growth
export async function fetchBalanceStatement(
  ticker,
  growth = false,
  period = 'annual'
) {
...
export async function fetchDiscountedCashflow(
  ticker,
  hist = false,
  period = 'annual'
) {
...
export async function fetchStockNews(query, limit = 50) {
  ...
  const link = getFMPLink(
    'stock_news',
    '',
    `tickers=${searchQuery}&limit=${limit}`
  );
  return await fetchData(link);
}
...
export async function getTickerResults() {
  const { assets, liabilities, commonstocksharesoutstanding } =
    await getLocalData(
      ['assets', 'liabilities', 'commonstocksharesoutstanding'],
      fetchFullStatement,
      [false, 'annual'],
      ['shortassetsannual', 'shortliabilitiesannual', 'sharesannual']
    );
  ...
  const roicTTM = await getLocalData(
    'roicTTM',
    fetchKeyMetrics,
    [true],
    'roicTTM'
  ...
  console.log('commonstocksharesoutstanding:', commonstocksharesoutstanding);
  console.log('freeCashFlow:', freeCashFlow);
  console.log('netIncome:', netIncome);
  console.log('revenue:', revenue);
  console.log('roicTTM:', roicTTM);
  console.log('priceEarningsRatio:', priceEarningsRatio);
  console.log('priceToFreeCashFlowsRatio:', priceToFreeCashFlowsRatio);
  console.log('--------------------');
  //! Remove
...
//* const shares = getLocalData(
//*   'commonstocksharesoutstanding',
//*   fetchFullStatement,
//*   [false, 'annual'],
//*   'sharesannual'
//* ).values
//* const assets = getLocalData(
//*   'assets',
//*   fetchFullStatement,
//*   [false, 'annual'],
//*   'assetsannual'
//* ).values
//* const liabilities = getLocalData(
//*   'liabilities',
//*   fetchFullStatement,
//*   [false, 'annual'],
//*   'liabilitiesannual'
...
//* const fcf = getLocalData(
//*   'freeCashFlow',
//*   fetchCashflowStatement,
//*   [false, 'annual'],
//*   'fcfannual'
//* ).values
//* const netIncome = getLocalData(
//*   'netIncome',
//*   fetchCashflowStatement,
//*   [false, 'annual'],
//*   'netincomeannual'
...
//* const roic = getLocalData('roicTTM', fetchKeyMetrics, [true], 'roicTTM').values

//* const pe = getLocalData('priceEarningsRatio', fetchRatios, [false, 'annual'], 'peannual').values
//* const pfcf = getLocalData(
